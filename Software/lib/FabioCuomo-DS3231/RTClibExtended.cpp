...
void RTC_DS3231::setAlarm(Ds3231_ALARM_TYPES_t alarmType, byte seconds, byte minutes, byte hours, byte daydate){
    ...
    if (alarmType & 0x01) seconds |= bit(A1M1);
    if (alarmType & 0x02) minutes |= bit(A1M2);
    if (alarmType & 0x04) hours |= bit(A1M3);
    if (alarmType & 0x10) hours |= bit(DYDT);
    if (alarmType & 0x08) daydate |= bit(A1M4);
